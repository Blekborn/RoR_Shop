class ProductsController < ApplicationController
  ...
  def index
    @products = if params[:q]
                  Product.search(params[:q]).order('created_at DESC').paginate(page: params[:page], per_page: 9)
                else
                  Product.all.order('created_at DESC').paginate(page: params[:page], per_page: 8)
                end
    @products = Product.order("#{sort_column} #{sort_direction}").paginate(page: params[:page], per_page: 9)
